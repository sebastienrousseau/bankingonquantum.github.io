---
name: "Banking On Quantum"
short_name: "bankingonquantum"
logo: "https://cloudcdn.pro/bankingonquantum/v1/logos/bankingonquantum.svg"
title: "Advisory & Engagement Tiers | Banking On Quantum"
description: "Engagement tiers for financial institutions, from open quantum research to PQC cryptographic audits and enterprise co-development."
keywords: "quantum banking advisory, PQC consulting, quantum risk audit"
author: "The Banking On Quantum Team"
date: "2026-09-01"
language: "en-GB"
layout: "pricing"
permalink: "https://bankingonquantum.com/pricing/index.html"
banner: "https://cloudcdn.pro/bankingonquantum/v1/github/github-bankingonquantum.svg"
banner_alt: "Banking On Quantum — Post-Quantum Cryptography and Quantum Computing in Finance"
nav_pricing: true
---

# Advisory & Technical Engagement

Partner with our team of quantum physicists, cryptographers, and financial engineers to achieve quantum readiness.

## Engagement Models

- **Open Research Tier**: Full access to published post-quantum benchmarks, algorithm repositories, and regulatory whitepapers.
- **PQC Cryptographic Discovery & Audit**: 6-week discovery engagement identifying all vulnerable cryptography across institutional infrastructure.
- **Enterprise Quantum Co-Development**: Dedicated technical sprints building tailored quantum algorithms for derivatives pricing or collateral optimization.
